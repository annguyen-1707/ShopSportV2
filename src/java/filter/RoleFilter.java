...
public class RoleFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        if (session.getAttribute("account") == null) {
            ...
            if (a.getRole() == 1) {
                System.out.println("Before forwarding to seller.jsp");
                req.getRequestDispatcher("sell/seller.jsp").forward(request, response);
                System.out.println("This line should not execute after forwarding");
                return; // Dừng luồng xử lý sau khi forward
            } else {
                res.sendRedirect("dined");
            }
        }

        // Chỉ tiếp tục chuỗi filter nếu không bị redirect hoặc forward
